...
export function StoryScrollSection() {
  ...
  // indices & range helpers for static headings
  const entStart = 1
  const entEnd = entStart + entrepreneurs.length - 1
  const comStart = entEnd + 1
  const comEnd = comStart + companies.length - 1
  ...
  const EDGE_TOL = 6         // px tolerance at edges
  const EXIT_AFTER_VH = 70   // extra natural scroll after last step (vh)
  ...
  useEffect(() => {
    const onWheel = (e: WheelEvent) => {
      const { top, bottomSnap, vh } = layoutRef.current
      const y = window.scrollY
      ...
      const lastSnapTop = bottomSnap - vh

      // let page continue above/below
  ...
  // ----- sync when user drags scrollbar / keyboard -----
  useEffect(() => {
    const onScroll = () => {
      const { top, bottomSnap } = layoutRef.current
      const y = window.scrollY
  ...
  const inEntRange = activeStep >= entStart && activeStep <= entEnd
  const inComRange = activeStep >= comStart && activeStep <= comEnd
  ...
  return (
    // Height = steps*100vh + exit buffer (so last item ke baad natural page scroll continue)
    <section
      ref={sectionRef}
    ...
    >
      {/* Sticky stage visible for first steps.length * 100vh */}
      <div className="sticky top-0 h-screen overflow-hidden bg-gradient-to-b from-background via-blue-50/30 to-background">
        <div className="relative w-full h-full flex items-center justify-center">
          {/* ==== STATIC HEADINGS (don’t animate) ==== */}
          {/* Entrepreneurs heading (shown during ent steps only) */}
          <div
            className="absolute top-10 md:top-14 left-0 right-0 flex justify-center"
            style={{ opacity: inEntRange ? 1 : 0, transition: "none", pointerEvents: "none" }}
          >
            <h3 className="text-2xl font-bold text-foreground md:text-4xl lg:text-5xl text-center">
              We have helped{" "}
          ...
          {/* Companies heading (shown during company steps only) */}
          <div
            className="absolute top-10 md:top-14 left-0 right-0 flex justify-center"
            style={{ opacity: inComRange ? 1 : 0, transition: "none", pointerEvents: "none" }}
          >
            <h3 className="text-2xl font-bold text-foreground md:text-4xl lg:text-5xl text-center">
              We empower{" "}
          ...
          {/* Entrepreneurs (3 snaps) — no heading inside */}
          {entrepreneurs.map((p, i) => {
            ...
            return (
              <div key={p.name} className="absolute inset-0 flex items-center justify-center px-4" style={screenStyle(stepIndex)}>
                <div className="flex flex-col items-center justify-center w-full max-w-6xl pt-24 md:pt-28">
                  <div className="relative w-full max-w-[300px] md:max-w-sm lg:max-w-lg h-[420px] md:h-[460px] flex items-center justify-center">
                    <div
                    ...
                    >
                      <div className="rounded-2xl max-w-[300px] md:max-w-sm lg:max-w-lg h-[420px] md:h-[460px] bg-white p-6 md:p-8 shadow-2xl border border-border w-full">
          ...
          {/* Companies (3 snaps) — no heading inside */}
          {companies.map((c, i) => {
              ...
              <div key={c.name} className="absolute inset-0 flex items-center justify-center px-4" style={screenStyle(stepIndex)}>
                <div className="flex flex-col items-center justify-center w-full max-w-6xl pt-24 md:pt-28">
                  <div className="relative w-full max-w-[300px] md:max-w-sm lg:max-w-lg h-[420px] md:h-[460px] flex items-center justify-center">
                    ...
                    >
                      <div className="rounded-2xl max-w-[300px] md:max-w-sm lg:max-w-lg h-[420px] md:h-[460px] bg-white p-6 md:p-8 shadow-2xl border border-border flex flex-col items-center justify-center aspect-square w-full">
          ...
          {/* Outro */}
          <div className="absolute inset-0 flex items-center justify-center px-4" style={screenStyle(steps.length - 1)}>
